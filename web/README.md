...
|---|---|
| `""` (default) | API and web are served from the same origin. For example nginx serves `/` from here and reverse-proxies `/api/` and `/health` to the API. **Preferred:** no CORS, no per-env config at all. |
