...
**DEVOPS**. It has everything you need to package, run, and deploy it without reading
the source.
...
| `LINKR_GIT_SHA` | set it in the build | `unknown` | Echoed on `/health`. **Please wire this to the real commit SHA.** It is how we tell what is actually deployed. |
...
> enough the moment we change a column. `S1P1/DEV-02` tracks replacing it with Alembic.
...
You can run the same suite against real Postgres. Worth adding as a separate
